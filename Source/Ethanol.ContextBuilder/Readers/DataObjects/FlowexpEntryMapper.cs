...
{
    public static class FlowexpEntryMapper
    {
        static IMapper _mapperflow = new MapperConfiguration(cfg =>
        {
            cfg.CreateMap<FlowexpEntry, IpFlow>()
            ...
            cfg.CreateMap<FlowexpEntry, DnsFlow>()
                .IncludeBase<FlowexpEntry, IpFlow>()
            ...
            cfg.CreateMap<FlowexpEntry, HttpFlow>()
                 .IncludeBase<FlowexpEntry, IpFlow>()
            ...
            cfg.CreateMap<FlowexpEntry, TlsFlow>()
                 .IncludeBase<FlowexpEntry, IpFlow>()
        ...
        /// <summary>
        /// Gets client to server <see cref="IpFlow"/> from the current <see cref="FlowexpEntry"/>.
        /// </summary>
        /// <param name="entry">The nfdump flow.</param>
        /// <returns><see cref="IpFlow"/> created from the current <see cref="FlowexpEntry"/>.</returns>
        public static IpFlow ToFlow(this FlowexpEntry entry) => Convert.ToApplicationProtocol(entry.NbarName) switch
        {
            ApplicationProtocols.DNS => _mapperflow.Map<FlowexpEntry, DnsFlow>(entry),
            ApplicationProtocols.HTTP => _mapperflow.Map<FlowexpEntry, HttpFlow>(entry),
            ApplicationProtocols.SSL => _mapperflow.Map<FlowexpEntry, TlsFlow>(entry),
            _ => _mapperflow.Map<FlowexpEntry, IpFlow>(entry)
        };
    }
}
